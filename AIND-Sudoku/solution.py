...
diagonal_unit_top_bottom = [r+cols[i] for i, r in enumerate(rows)]
diagonal_unit_bottom_top = [r+cols[i] for i, r in enumerate(rows[::-1])]
diagonal_units = [diagonal_unit_top_bottom, diagonal_unit_bottom_top]
...
def naked_twins(values):
    ...
    for unit in unitlist:
        ...
        naked_twins = {twin: values[twin] for twin in unit if
                       unit_values.count(values[twin]) == 2 and
                       len(values[twin]) == 2}
        # Eliminate the naked twins as possibilities for their peers
        peers = [box for box in unit if box not in naked_twins.keys()]
        for val in set(naked_twins.values()):
            for peer in peers:
