...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        # Add new action level and init to empty set
        self.a_levels.append(set())
        parents = self.s_levels[level]

        # Iterate through all possible actions
        for action in self.all_actions:
            child = PgNode_a(action)
            # Check if the child's preconds are satisfied
            if child.prenodes.issubset(parents):
                # Iterate through the parent nodes(aka the preconds of the
                # child node) to find all preconds for the child
                for parent in parents:
                    if parent in child.prenodes:
                        # Connect the child node and the parent node
                        parent.children.add(child)
                        child.parents.add(parent)
                        # Append the created node to the action list
                        self.a_levels[level].add(child)
    ...
    def add_literal_level(self, level):
        ...
        # Add a literal level and init to empty set
        self.s_levels.append(set())
        # Grab previous level A nodes
        parents = self.a_levels[level - 1]

        # Iterate through previous level A nodes
        # Adding their effects nodes(children) to the newly created S level
        # Also maintaining connections between parent and child nodes
        for parent in parents:
    ...
    def inconsistent_effects_mutex(
        ...
        :return: bool
        """
        # Test whether one action negates the effects of the other
        # postiive literals in PgNode_a.effect_add and negative literals in
        # PgNode_a.effect_rem
        return not set(
            node_a1.action.effect_add).isdisjoint(
            node_a2.action.effect_rem) or not set(
            node_a1.action.effect_rem).isdisjoint(
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        # Test if the effect of one action is a negation of a precond in the
        # other

        # Test if positive literals in either action a1/a2 are negated in their
        # opposing action's a2/a1 positive preconditions
        pos_1 = set(a1.effect_add).isdisjoint(a2.precond_pos)
        pos_2 = set(a2.effect_add).isdisjoint(a1.precond_pos)

        # Test if negative literals in either action a1/a2 are negated in their
        # opposing action's a2/a1 negative preconditions
        neg_1 = set(a1.effect_rem).isdisjoint(a2.precond_neg)
        neg_2 = set(a2.effect_rem).isdisjoint(a1.precond_neg)
        return not(pos_1 and pos_2 and neg_1 and neg_2)
    ...
    def competing_needs_mutex(
        ...
        :return: bool
        """
        # Preconditions of the action nodes are its parent PgNode_s nodes
        # So we can iterate through the parent nodes. If we find a mutex
    ...
    def update_s_mutex(self, nodeset: set):
        ...
        for i, n1 in enumerate(nodelist[:-1]):
            for n2 in nodelist[i + 1:]:
                if self.negation_mutex(
                        n1,
                        n2) or self.inconsistent_support_mutex(
                        n1,
                        n2):
                    mutexify(n1, n2)
    ...
    def negation_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s) -> bool:
        ...
        :return: bool
        """
        # First we need to check if the literals(symbols) are the same then we
        # can check if their negations of one another
        return node_s1.symbol == node_s2.symbol and node_s1.is_pos != node_s2.is_pos

    def inconsistent_support_mutex(
            self,
            node_s1: PgNode_s,
            node_s2: PgNode_s) -> bool:
        """
        Test a pair of state literals for mutual exclusion, returning True if
        ...
        :return: bool
        """
        # Similiar to competing_needs_mutex but instead we iterate through
        # parent action nodes. We return False if we find two actions that
        # aren't mutex that can achieve the two literals.
        for a_s1 in node_s1.parents:
            for a_s2 in node_s2.parents:
    ...
    def h_levelsum(self) -> int:
        ...
        """
        level_sum = 0
        # Convert all (pos) goal literals into PgNode_s
        goals = [PgNode_s(g, True) for g in self.problem.goal]

        # The level in which a goal literal first appears is its level cost
        # We can then return the sum of the goal's level cost

        # Iterate through all the literals at each s level
        for level, state in enumerate(self.s_levels):
            # Search for goal literals that can be found at the given state
            # level
            for goal in goals:
                # If found, add the level number to sum and remove goal from
                # goals list
                if goal in state:
                    level_sum += level
                    goals.remove(goal)
